...
import { X, ArrowRight, ArrowUpRight, ShieldCheck, HardHat, Landmark, Layers, FileSpreadsheet, Building2 } from 'lucide-react'
...
const BEAM_TILES: NavItem[] = [
  {
    label: 'Admin Login',
    href: '/login?next=/portal/admin/landing',
...
export function BeamGroundsNav({
  ...
  const groundsTiles: NavItem[] = groundsLinks
    ? groundsLinks.map((link) => {
        ...
        if (link.label.includes('01')) {
          ...
          subtitle = 'Participant hours, worklogs & crew allocations'
          badge = 'Roster'
          icon = HardHat
        } else if (link.href.includes('admin')) {
          subtitle = 'Landing showcase & operating loop administration'
          badge = 'Command'
          icon = ShieldCheck
        }
  ...
  return (
    <>
      ...
      {/* Mobile Full-Screen Transparent Tiles Menu */}
      {open !== null ? (
        <div
          role="dialog"
          aria-modal="true"
          className="fixed inset-0 z-50 flex flex-col justify-between overflow-y-auto bg-[#07100c]/95 p-6 backdrop-blur-2xl sm:hidden select-none animate-in fade-in duration-200"
        >
          {/* Subtle Ambient Glow */}
